...
int main(int argc, char const* argv[])
{
	...
	try
	{
		...
		double totalTimeVec = static_cast<double>(endTimeVec - startTimeVec)  / CLOCKS_PER_SEC * 1000000; // us（マイクロ秒）単位
		...
		double totalTimeList = static_cast<double>(endTimeList - startTimeList)  / CLOCKS_PER_SEC * 1000000; // us（マイクロ秒）単位
